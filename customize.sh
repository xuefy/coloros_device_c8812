...
rm -f $DIR/new-update/system/app/OppoFlashLight.apk
mkdir -p $DIR/new-update/data/system_app
for FILE in `cat $CUSTDIR/remove_to_data.txt | grep -v ^# | grep -v ^$`; do
	mv $DIR/new-update/system/app/$FILE $DIR/new-update/data/system_app/
done
